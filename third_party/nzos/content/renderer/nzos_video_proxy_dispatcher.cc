...
#include "nz_video_proxy_dispatcher.h"
...
#include "nzos/ozone/nzos_platform_thread.h"
...
NzMsgDelay::NzMsgDelay(int request_id, base::Time last_time, IPC::Message* msg, uint64 bits) :
    request_id_(request_id), insert_time_(base::Time::Now()), last_time_(last_time), msg_(msg), bits_(bits)
{}
...
NzVideoProxyDispatcher::NzVideoProxyDispatcher(
        ...
        ResourceDispatcher* resource_dispatcher, IPC::Sender* sender) :
        sender_(sender), ipc_task_runner_(ipc_task_runner),
                resource_dispatcher_(resource_dispatcher), capabilities_rcvd_(false), h264_capable_(false),
                vp8_capable_(false), vp9_capable_(false), clearkey_capable_(false),
                nz_encrypt_capable_(false), widevine_capable_(false), render_id_(-1), active_nz_decoders_(0),
                target_bw_(TARGETBW), current_bw_(TARGETBW), reset_bw_calc_(false), detect360VideoByCanvas_(false) {
...
void NzVideoProxyDispatcher::OnFilterAdded(IPC::Sender* sender) {
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
}
...
void NzVideoProxyDispatcher::OnCapabilities(
    ...
    LOG(INFO) << "NzVideoProxyDispatcher::OnCapabilities: "
        << "BypassUrl: " << bypass_url_
        << ", BypassAttr: " << bypass_attr_;
}

void NzVideoProxyDispatcher::OnClientBandwidth(const Nz_Client_Bandwidth& bandwidth_data) {
...
void NzVideoProxyDispatcher::OnKeyRequest(const Nz_Key_Request& key_request) {
	media::NzDecryptor* decryptor = media::NzDecryptor::getNzDecryptor(
			key_request.id);
	if (decryptor) {
		return decryptor->KeyRequest(key_request.id, key_request.key_rqst_id,
				key_request.opaque_data, key_request.url);
	}
	media::NzWvDecryptor* wv_decryptor = media::NzWvDecryptor::getNzWvDecryptor(
			key_request.id);
	if (wv_decryptor) {
		return wv_decryptor->KeyRequest(key_request.id, key_request.key_rqst_id,
				key_request.opaque_data, key_request.url);
	}
}
    ...
    if (resource_dispatcher_ && (active_nz_decoders_ > 0)) {
        ...
        if (it == bw_data_.end()) {
            ...
            if (!last_time.is_null() && (data_length != 0)) {
                base::TimeDelta time_delta = base::Time::Now() - curr_entry->last_time_;
                ...
                if (td != 0) {
                    ...
                    //                        << rate << " bits/sec -> "
                    //                        << bits << "/" << td;
                    //LOG(ERROR) << "current BW: " << curr_entry->current_bw_ << " new BW: " << new_bw;
                    ...
                    if (curr_entry->data_points_ > DATAPOINTS) {
                        ...
                        if ((curr_entry->data_points_ % DATAPOINTS) == 0) {
                            //LOG(ERROR) << "current BW: " << current_bw_ << " new BW: " << curr_entry->current_bw_;
                        }
                    }
                }
            } else {
        ...
        if (rrDelay_.size() > 0) {
            ...
            NzMsgDelay* md = new NzMsgDelay(request_id, last_time, new IPC::Message(message), bits);
            ...
            if (delay > 0) {
                ...
                //LOG(ERROR) << "Stealing the ResourceMsg_DataReceived: (" << request_id << ") "
                //        << delay << ", " << data_length;

                timer_.reset(
                        new base::OneShotTimer<NzVideoProxyDispatcher>());
                timer_->Start(
                        FROM_HERE, base::TimeDelta::FromMicroseconds(delay), this, &NzVideoProxyDispatcher::PostMsg);
    ...
    if (resource_dispatcher_ && active_nz_decoders_ > 0) {
        ...
        if (it != bw_data_.end()) {
            if ((rrDelay_.size() > 0)) {
                ...
                NzMsgDelay* md = new NzMsgDelay(request_id, base::Time(), new IPC::Message(message), 0);
                rrDelay_.push_back(md);
    
                //LOG(ERROR) << "Stealing the ResourceMsg_RequestComplete: (" << request_id << ")";
...
void NzVideoProxyDispatcher::PostMsg() {
    ...
    if (rrDelay_.size() > 0) {
        ...
        if (md) {
            ...
            //LOG(ERROR) << "Returning the Message: (" << request_id << ") Type: " <<
            //        (m->type() == ResourceMsg_DataReceived::ID ? "DataReceived" : "RequestComplete");
            ...
            if (m->type() == ResourceMsg_RequestComplete::ID) {
                ...
                if (!md->last_time_.is_null() && (md->bits_ != 0)) {
                    base::TimeDelta time_delta = base::Time::Now() - md->last_time_;
                    ...
                    if (td != 0) {
                        ...
                        counter++;
                        if ((counter % 10) == 0)
                            LOG(ERROR) << "Apparent rate: " << rate << " td: " << td << " bits_ " << md->bits_;
                    }
                }
#else
            ...
            while (rrDelay_.size() > 0) {
                ...
                if (md->request_id_ != request_id) {
                    ...
                    if (!it->second->last_sent_time_.is_null()) {
                        base::TimeDelta td = base::Time::Now() - it->second->last_sent_time_;
                        delay -= td.InMicroseconds();
                    }
                }
                ...
                if (delay <= 0) {
                    ...
                    timer_.reset(new base::OneShotTimer<NzVideoProxyDispatcher>());
                    timer_->Start(
                            FROM_HERE, base::TimeDelta::FromMicroseconds(delay), this, &NzVideoProxyDispatcher::PostMsg);
                    break;
                }
...
void NzVideoProxyDispatcher::BoundingRect(
    ...
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::BoundingRectOnIOThread, this, bounding_rect));
}
...
void NzVideoProxyDispatcher::AudioCreate(const Nz_Proxy_Create& create_data)  {
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::AudioCreateOnIOThread, this, create_data));
}

void NzVideoProxyDispatcher::AudioStart(const Nz_Audio_Initial_Data& init_data) {
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::AudioStartOnIOThread, this, init_data));
}
...
void NzVideoProxyDispatcher::AudioVolume(const Nz_Audio_Volume& volume_data) {
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::AudioVolumeOnIOThread, this, volume_data));
}

void NzVideoProxyDispatcher::AudioDestroy(const Nz_Proxy_Id& id_data) {
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::AudioDestroyOnIOThread, this, id_data));
}

void NzVideoProxyDispatcher::AudioCreateOnIOThread(const Nz_Proxy_Create& create_data) {
	Send(new NzVideoProxyHostMsg_AudioCreate(create_data));
}
void NzVideoProxyDispatcher::AudioStartOnIOThread(const Nz_Audio_Initial_Data& init_data) {
	Send(new NzVideoProxyHostMsg_AudioStart(init_data));
}
void NzVideoProxyDispatcher::AudioBufferOnIOThread(const Nz_Proxy_Media_Buffer& buffer) {
	Send(new NzVideoProxyHostMsg_AudioBuffer(buffer));
}
void NzVideoProxyDispatcher::AudioVolumeOnIOThread(const Nz_Audio_Volume& volume_data) {
	Send(new NzVideoProxyHostMsg_AudioSetVolume(volume_data));

}void NzVideoProxyDispatcher::AudioDestroyOnIOThread(const Nz_Proxy_Id& id_data) {
...
void NzVideoProxyDispatcher::CreateDecryptor(const Nz_Decrypt_Create& create_data) {
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::CreateDecryptorOnIOThread, this, create_data));
}

void NzVideoProxyDispatcher::GenerateKeyRequest(const Nz_Generate_Key_Request& request_data) {
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::GenerateKeyRequestOnIOThread, this, request_data));
}

void NzVideoProxyDispatcher::UpdateSession(const Nz_Key_Data& key_data) {
    ipc_task_runner_->PostTask(FROM_HERE,
    base::Bind(&NzVideoProxyDispatcher::UpdateSessionOnIOThread, this, key_data));
}

void NzVideoProxyDispatcher::ReleaseSession(const Nz_Session_Release& session_data) {
	ipc_task_runner_->PostTask(FROM_HERE,
	    base::Bind(&NzVideoProxyDispatcher::ReleaseSessionOnIOThread, this, session_data));
}

void NzVideoProxyDispatcher::CreateDecryptorOnIOThread(const Nz_Decrypt_Create& create_data) {
	Send(new NzVideoProxyHostMsg_DecryptorCreate(create_data));
}

void NzVideoProxyDispatcher::GenerateKeyRequestOnIOThread(const Nz_Generate_Key_Request& request_data) {
    Send(new NzVideoProxyHostMsg_GenerateKeyRequest(request_data));
}

void NzVideoProxyDispatcher::UpdateSessionOnIOThread(const Nz_Key_Data& key_data) {
	Send(new NzVideoProxyHostMsg_UpdateSession(key_data));
}

void NzVideoProxyDispatcher::ReleaseSessionOnIOThread(const Nz_Session_Release& session_data) {
...
{
	ipc_task_runner_->PostTask(FROM_HERE,
			base::Bind(&NzVideoProxyDispatcher::NzLogOnIOThread, this, severity, log_msg));
}

void NzVideoProxyDispatcher::NzLogOnIOThread(int severity, const std::string& log_msg)
{
	Send(new NzVideoProxyHostMsg_Log(severity, log_msg));
}

void NzVideoProxyDispatcher::OnDeviceProperties(bool allowUserAgentChange, bool isMobileDevice, std::string deviceSw)
{
  // Note: Only static functions of NzosPlatformThread are usable in the renderer
  ...
  LOG(INFO) << "Device Properties received: " << "Allow user Agent Change: " <<
      ((allowUserAgentChange) ? "Yes" : "No") << ", Is Mobile: " <<
      ((isMobileDevice) ? "Yes" : "No") << ", Device S/W: " << deviceSw;
}
double NzVideoProxyDispatcher::calcDelay(int64 bits) {

    double des_secs = ((double) bits) / target_bw_;

    double cur_secs = ((double) bits) / current_bw_;

    int64 delay = (des_secs - cur_secs) * base::Time::kMicrosecondsPerSecond;

    //LOG(ERROR) << "bytes: " << bits / 8 << " des_secs: " << des_secs << " cur_secs: " << cur_secs << " delay: " << delay;
